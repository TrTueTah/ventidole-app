...
import { blackColor, grayColor, secondaryColor, whiteColor } from 'constants/colors';
...
import { useRemoteConfigStore } from 'src/store/remoteConfig/remoteConfigStore';
...
export const TabIcon = (focused: boolean, route: string): ReactElement => {
  const { enableConnectionFeature } = useRemoteConfigStore();

  const getIcon = (): ReactElement =>
    ({
      [homePath]: focused ? <Home color={secondaryColor} /> : <Home color={blackColor} />,
      [shopPath]: focused ? <Shop color={secondaryColor} /> : <Shop color={blackColor} />,
      [archivePath]: focused ? <Archive color={secondaryColor} /> : <Archive color={blackColor} />,
      [chatPath]: focused ? <Chat color={secondaryColor} /> : <Chat color={blackColor} />,
      [morePath]: focused ? <More color={secondaryColor} /> : <More color={blackColor} />,
    }[route as keyof RootStackParamList]);
