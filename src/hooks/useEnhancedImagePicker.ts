...
export const useEnhancedImagePicker = () => {
    const { hideNavigationBarForCrop, showNavigationBarAfterCrop } = useAndroidImageCropNavigationBar();
    ...
    const openPickerWithAndroidFixes = async (options: any = {}) => {
        try {
            ...
            const platformConfig = Platform.OS === 'android' ? androidConfig : iosConfig;
            const config = {
                width: 1200,
                height: 1600,
                cropping: true,
    ...
    const openCropperWithAndroidFixes = async (path: string, options: any = {}) => {
        try {
